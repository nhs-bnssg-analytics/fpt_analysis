...
results <- readRDS(
  ...
  select(
    ...
    `MAE (test data)` = "Test set value",
    `MAE (baseline - last year)` = "Baseline (same as last year)",
    `MAE (baseline - linear 3 years)` = "Baseline (linear (3 years))"
  ) |> 
  mutate(
...
The tables in this section show, for each modelling method and outcome, the most important variables selected by the models for the models with the lowest MAE. 
...
for (i in seq_len(nrow(methods))) {
  ...
  results_per_method <- results |> 
    ...
    filter(
    unrounded_mae == min(unrounded_mae),
    .by = `Target variable`
...
@tbl-best-models-inputs presents the combinations of the inputs to the modelling that created the lowest MAE for each outcome, along with the baseline comparisons.

```{r}
#| label: tbl-best-models-inputs
#| tbl-cap: "By outcome, the best combination of input data, along with the MAE score when comparing the predicted outcome with the observed outcome on the data held back for testing."
#| echo: false

results |> 
  filter(
    `MAE (test data)`== min(`MAE (test data)`),
    .by = `Target variable`
